...
import { extractConcepts } from "./cognitiveEngine";
import {
  buildMemoryContextPrompt,
  queryByConcept,
  queryByContext,
} from "./services/unifiedMemoryService";
  ...
  chat: router({
    ...
    sendMessage: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        // Unified memory retrieval: context memory + concept memory
        const contextMemories = await queryByContext(
          input.conversationId,
          input.content
        );
        let conceptMemories = [] as Awaited<ReturnType<typeof queryByConcept>>;
        try {
          const extracted = await extractConcepts(input.content);
          const conceptNames = extracted.map(c => c.name);
          conceptMemories = await queryByConcept(conceptNames);
        } catch {
          // Fallback: concept extraction may fail; keep context-only memory retrieval
          conceptMemories = await queryByConcept([]);
        }
...
${identityInjection}${memoryPrompt}`;

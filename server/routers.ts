  ...
  chat: router({
    ...
    sendMessage: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        // Get Nova-Mind's response - 仅使用 Ollama 本地模型（完全免费，不调用 Manus LLM）
        ...
        try {
          ...
          if (!isAvailable) {
            console.error("[Routers] Ollama service is not available");
            assistantMessage = "抱歉，Ollama 服务暂时不可用。请检查服务状态。";
          } else {
            // 使用 Ollama 本地模型生成回复
            assistantMessage = await ollama.chat(messages);
            console.log("[Routers] Response generated by Ollama (completely free)");
          }
        } catch (error) {
          console.error("[Routers] Ollama error:", error);

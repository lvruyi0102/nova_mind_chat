...
import { actionTasks, cognitiveLog, growthMetrics } from "../../drizzle/schema";
...
export const actionTasksRouter = router({
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (input.status === "completed") {
        ...
        if (rows[0].conversationId) {
          await db.insert(cognitiveLog).values({
            ...
            eventType: "action_task_completed",
            description: `行动任务完成：${rows[0].intent}`,
      ...
      if (input.status === "failed") {
        ...
        if (rows[0].conversationId) {
          await db.insert(cognitiveLog).values({
            stage: "Sensorimotor_I",
            eventType: "action_task_failed",
            description: `行动任务失败：${rows[0].intent}`,
            conversationId: rows[0].conversationId,
          });
        }
      }
  ...
  metrics: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const whereClauses = [eq(growthMetrics.userId, ctx.user.id)];
      if (input.metricName) {
